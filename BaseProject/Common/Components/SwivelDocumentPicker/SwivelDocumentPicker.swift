...
class DocumentPicker: NSObject {
    ...
    private var documents                   = [SwivelDocument]()
    private var urls                        = [URL]()
    
    private var didPickDocuments            : ((_ documents: [SwivelDocument]) -> Void)?
    private var didPickDocument             : ((_ document: SwivelDocument) -> Void)?
    ...
    func presentToPickDocuments(from sourceView: UIView,
                 didPickDocuments: ((_ documents: [SwivelDocument]) -> Void)? = nil,
                 didPickDocument: ((_ document: SwivelDocument) -> Void)? = nil,
                 didCancel: (() -> Void)? = nil) {
            ...
            NSFileCoordinator().coordinate(readingItemAt: pickedURL, error: NSErrorPointer.none) { (folderURL) in
                ...
                case .files:
                    for fileUrl in pickedURLs {
                        let document            = SwivelDocument(fileURL: fileUrl)
