...
class BaseVC<ViewModel: BaseVM>: BaseSuperVC {
    ...
    func addBackButton(title: String? = "Back".localized(), icon: String? = "icon_back") {
        ...
        if let viewModel = viewModel {
            disposeBag.insert([
                ...
                viewModel.showAlert.subscribe(onNext: { [weak self] (alertTupple) in
                    ...
                    let cancelAction  = UIAlertAction(title: alertTupple.secondaryBtnTitle ?? "Cancel".localized(), style: .default, handler: alertTupple.secondaryAction != nil ? { (ac) in alertTupple.secondaryAction!() } : nil)
                    ...
                    if let action = alertTupple.primaryAction {
                        let primaryAction = UIAlertAction(title: alertTupple.primaryBtnTitle ?? "Confirm".localized(), style: .default, handler: { (ac) in action() })
