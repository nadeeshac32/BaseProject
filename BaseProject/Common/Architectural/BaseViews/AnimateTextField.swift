    ...
    public func validate(fieldName: String?, text: String?) -> [String]? {
        ...
        case .required:
            if let text = text?.trimmingCharacters(in: .whitespacesAndNewlines), text.isNotEmpty {
                ...
                return ["is required.".localizeWithFormat(arguments: "\(fieldName ?? "This field".localized())")]
            }
        case .email:
            ...
            if let text = text, emailRegex.matches(text) {
                ...
                return ["Invalid".localizeWithFormat(arguments: "\(fieldName?.lowercased() ?? "input".localized())")]
            }
        case .phone                                         :
        ...
        case let .confirmPassword(passwordField):
            ...
            if password.isEmpty {
                return ["Please enter password first.".localized()]
            }
        ...
        case .optional:
            if text == nil || text == "" {
                ...
                return ["field can be empty.".localizeWithFormat(arguments: "\(fieldName?.lowercased() ?? "This".localized())")]
            }
        case let .numberWithin(max, min):
            ...
            if let number = currencyFormatter.number(from: text ?? "") {
                if number.doubleValue < Double(min) {
                    return ["Value should be more than".localizeWithFormat(arguments: "\(min - 1)")]
                } else if number.doubleValue > Double(max) {
                    return ["Maximum value you can enter is".localizeWithFormat(arguments: "\(max)")]
                } else {
                    return nil
                }
            } else {
                return ["Invalid value".localized()]
            }
