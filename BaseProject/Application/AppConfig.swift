...
class AppConfig: NSObject, CLLocationManagerDelegate {
    ...
    var contactEmail: String! {
        guard let string        = getValue(key: "contactEmail") else { return "" }
        return (string as! String)
    }
    ...
    var urlCache: URLCache {
        let cache = URLCache(memoryCapacity: memoryCapacity, diskCapacity: diskCapacity, diskPath: "\(appName!)Diskpath")
        return cache
    }
